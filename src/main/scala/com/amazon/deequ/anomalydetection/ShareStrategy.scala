...
case class ShareStrategy(
  ...
  override def detect(
    ...
    val dataSeriesTemp: Vector[(Double, Double)] = dataSeries.drop(1).zip(dataSeries.view.dropRight(1))
    val dataSeriesDiff: Vector[Double] = dataSeriesTemp.map{case (i, j) => abs(i - j) }

    val mAV = if (includeInterval) {
      meanAndVariance(dataSeriesDiff)
    } else {
      val valuesBeforeInterval = dataSeriesDiff.slice(0, searchStart)
      val valuesAfterInterval = dataSeriesDiff.slice(searchEnd, dataSeriesDiff.length)
